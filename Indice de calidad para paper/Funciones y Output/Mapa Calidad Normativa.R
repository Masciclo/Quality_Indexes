MapaCalidadNormativa <- function(df) {
  ...
  mapacalidadobj <- ggplot() +
    ...
    geom_sf(data = op_ci_0, col= "#13D657", size = .6,
            alpha = 0.6) +
    geom_sf(data = op_ci_1, col= "#D6372B", size = .6,
            alpha = 0.6) +
    #Barra de escala
    ...
    #Titulo
    ggtitle("Santiago", subtitle = paste0("(Calidad respetando normativa: ","Reprobadas = ", Reprobadas,"%, Aprobadas = ", Aprobadas, "%)")) +

    #Cordenadas
    coord_sf(crs = st_crs(4326), xlim = bbx[1,], ylim = bbx[2,],
             expand = FALSE,label_axes = element_blank(), datum = NA) +
    theme_bw()
 ggsave("output/mapa calidad normativa/Mapa_calidad.png"
